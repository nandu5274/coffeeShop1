...
export class MembershipCreateComponent {
  ...
  sendMemberShipRRegistrationOtpThroughMail(otp: any, username: any, mailId:any) {
    let request: any = {};
    request.recipient = mailId;
    request.msgBody = "Hey! " + username + "\nThis is a message from the cafe kubera here is the otp: " + otp + " \n\nThanks";
    request.subject = otp + " is OTP for the Membership Registration ";
    this.dataService.SendSimpleMail(request).subscribe();
    this.otpSentBanner = true
  }
  ...
  clearAllBanners() {
    ...
    this.invalidOtpBanner=false
    this.otpSentBanner=false
    this.memberShipOtpSuccessBanner = false
  }
  backToSignup() {
    this.clearAllBanners();
...
memberShipOtp:any;
member_ship_otp_field:any=''
invalidOtpBanner:boolean=false
otpSentBanner:boolean=false
memberShipOtpSuccessBanner:boolean=false
  sendOtpForMembership()
  {
    this.memberShipOtp = Math.floor(Math.random() * 9000) + 1000;
    this.sendMemberShipRRegistrationOtpThroughMail(this.memberShipOtp, this.customerDetails.customer_detail.name, this.customerDetails.customer_detail.email_id);
  }

  createMemberShip() {
    this.clearAllBanners()
    if(this.member_ship_otp_field==this.memberShipOtp)
    {
      //create membership
    this.createMemberShipForCustomer();
    }else{
      this.invalidOtpBanner = true;
  ...
  createMemberShipForCustomer()
  {
    ...
    const formattedDate = currentDate.toISOString().split('T')[0];
      request.customer_details_id = this.customerDetails.customer_detail.id
      request.created_date = formattedDate
      request.validity_month = this.validity_month
      request.expiry_date = this.getExpireDate(this.validity_month)
      request.created_by = this.getCapUser()
    this.customerService.createCustomerMemberShip(request).subscribe(
      (result: any) => {
        ...
        if (result.errors && result.errors.length > 0) {
          ...
          if (result.data.insert_kubera_profile_customer_member_ship_one) {
  ...
  getExpireDate(validity_month:any) {
    const currentDate = new Date();
    const expirationMonths = parseInt(validity_month); // Number of months to add to the current date
